...
function createSimpleBox(ed, writer, selection, val) {
  const simpleGrid = writer.createElement('simpleGrid', {"attr": val.text});
  const simpleGridLayout1 = writer.createElement('simpleGridLayout', {
    "class": val.class,
    "class_left_column": val.left_class
  });
  const simpleGridLayout2 = writer.createElement('simpleGridLayout', {
    "class": val.class,
    "class_right_column": val.right_class
  });
  if (val.key === 'three-col') {
    const simpleGridLayout3 = writer.createElement('simpleGridLayout', {"class": val.class});
    writer.append(simpleGridLayout3, simpleGrid);
    writer.appendElement('paragraph', simpleGridLayout3);
  }
