...
namespace Drupal\cpce_profiles\Controller;
...
class ProfileAPIController extends ControllerBase {
  ...
  public function fetchProfileapi() {
    ...
    \Drupal::logger('my_module')->notice('The value of $response is @path', ['@path' => $response]);
    curl_close($curl);
    print_r($response);
  ...
  public function updateProfiles($api_profiles, $node_data) {
    foreach ($node_data as $nid => $person_id) {
      ...
      $node->set('field_team_member_external', $api_profiles[$person_id]['cta_link']);
  ...
  public function createProfiles($profiles, $api_profiles) {
    foreach ($api_profiles as $person_id) {
      $node = Node::create([
        ...
        'field_team_member_external' => $profiles[$person_id]['cta_link'],
